...
class BoostPlanResponse(BaseModel):
    id: str
    name: str
...
async def get_boost_plans(
    ...
    """
    # Get all active plans directly
    query = select(BoostPlan).where(BoostPlan.status == "active").order_by(BoostPlan.price)
    result = await db.execute(query)
    plans = result.scalars().all()
    print(f"DEBUG: Found {len(plans)} active plans")
    ...
    return [
        BoostPlanResponse(
            ...
            applicable_to=p.applicable_to.value if hasattr(p.applicable_to, 'value') else str(p.applicable_to),
            placement=p.placement.value if hasattr(p.placement, 'value') else str(p.placement),
...
async def mark_request_paid(
    ...
    """Mark a boost request as paid. Payment does NOT activate - admin approval needed."""
    ...
    return {"message": "Payment recorded. Pending admin approval."}
