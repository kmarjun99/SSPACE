...
export const venueService = {
    ...
    getAllReadingRooms: async (): Promise<ReadingRoom[]> => {
        ...
        return response.data.map((room: any) => ({
            ...
            images: room.images, // JSON string
    ...
    getAllCabins: async (): Promise<Cabin[]> => {
        ...
        return response.data.map((cabin: any) => ({
            ...
            amenities: Array.isArray(cabin.amenities) ? cabin.amenities : (typeof cabin.amenities === 'string' ? cabin.amenities.split(',') : []),
        }));
    },
    ...
    createReadingRoom: async (data: Partial<ReadingRoom>): Promise<ReadingRoom> => {
        const payload: any = {
            ...
            images: data.images, // Expecting JSON string already
        ...
        return {
            ...
            priceStart: room.price_start,
            contactPhone: room.contact_phone,
            amenities: Array.isArray(room.amenities) ? room.amenities : (typeof room.amenities === 'string' ? room.amenities.split(',') : []),
            status: room.status,
            isVerified: room.is_verified
        };
    },
    ...
    updateReadingRoom: async (id: string, updates: Partial<ReadingRoom>): Promise<ReadingRoom> => {
        ...
        if (updates.amenities !== undefined) {
            payload.amenities = Array.isArray(updates.amenities)
                ? updates.amenities.join(',')
                : updates.amenities;
        }
    ...
    createCabin: async (readingRoomId: string, data: Partial<Cabin>): Promise<Cabin> => {
        ...
        return {
            ...cabin,
            readingRoomId: cabin.reading_room_id,
            amenities: Array.isArray(cabin.amenities)
                ? cabin.amenities
                : (typeof cabin.amenities === 'string' ? cabin.amenities.split(',') : []),
        };
    },
