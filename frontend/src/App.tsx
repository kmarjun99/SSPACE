...
const App: React.FC = () => {
  ...
  const [appState, setAppState] = useState<AppState>(() => {
    ...
    // Load persisted app data if available to prevent data loss on refresh
    const savedDataJson = localStorage.getItem('studySpace_data');
    const savedData = savedDataJson ? JSON.parse(savedDataJson) : {};

    return {
      currentUser: savedUser,
      users: MOCK_USERS,
      readingRooms: [], // Start empty - backend is source of truth
      // Use saved data or fallback to mocks
      cabins: [], // Reset
      bookings: MOCK_BOOKINGS,
      notifications: [], // Reset
      reviews: [], // Reset
      waitlist: [], // Reset
      accommodations: [], // Start empty - backend is source of truth
      favorites: [], // Start empty - will be fetched from API
  ...
  useEffect(() => {
    // Only persist essential frontend state, not backend data
    const dataToSave = {
      bookings: appState.bookings,
      notifications: appState.notifications,
      reviews: appState.reviews,
      waitlist: appState.waitlist,
      tickets: appState.tickets,
      settings: appState.settings
      // Note: cabins, accommodations, readingRooms come from backend - don't persist
    };
    try {
      localStorage.setItem('studySpace_data', JSON.stringify(dataToSave));
    } catch (e) {
      // Handle QuotaExceededError - clear old data and retry once
      console.warn('localStorage quota exceeded, clearing and retrying...', e);
      try {
        localStorage.removeItem('studySpace_data');
        localStorage.setItem('studySpace_data', JSON.stringify(dataToSave));
      } catch (e2) {
        console.error('Failed to save to localStorage even after clearing:', e2);
      }
    }
  }, [appState.bookings, appState.notifications, appState.reviews, appState.waitlist, appState.tickets, appState.settings]);
