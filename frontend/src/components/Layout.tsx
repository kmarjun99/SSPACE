...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, notifications, onMarkNotificationRead }) => {
  ...
  const NavItem: React.FC<{ item: any }> = ({ item }) => {
    ...
    return (
      <Link to={item.href} className={`${baseClass} ${activeClass}`}>
        <item.icon className={`mr-3 flex-shrink-0 h-5 w-5 ${isActive ? 'text-white' : 'text-indigo-300 group-hover:text-white'}`} />
        {item.name}
      </Link>
    );
  };
          ...
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className={`p-1 rounded-full transition-all ${isActive ? 'bg-indigo-50 translate-y-[-2px]' : ''}`}>
