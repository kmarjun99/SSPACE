        ...
        const fetchAccommodations = async () => {
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    // Fallback to mock data from state if backend returns empty
                    setAccommodations(state.accommodations);
                }
            } catch (error) {
                console.warn('Failed to fetch accommodations from backend, using cached data:', error);
                setFetchError('Unable to load latest listings. Showing cached data.');
                setAccommodations(state.accommodations);
            } finally {
                setIsLoading(false);
            }
