...
export const SuperAdminReadingRoomReview = () => {
    ...
    if (error || !venue) return <div className="p-10 text-center text-red-500">{error || 'Venue not found'}</div>;

    // Helper to parser images
    const images = venue.images ? JSON.parse(venue.images) : venue.imageUrl ? [venue.imageUrl] : [];
                    ...
                    <Card className="p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-medium">{venue.amenities.length} Included</span>
                            </div>
                            <div className="flex flex-wrap gap-1 mt-2">
                                {venue.amenities.slice(0, 5).map((am, i) => (
                                    <span key={i} className="text-[10px] bg-gray-100 px-2 py-1 rounded text-gray-600">{am}</span>
                                ))}
                                {venue.amenities.length > 5 && <span className="text-[10px] text-gray-400">+{venue.amenities.length - 5} more</span>}
                            </div>
                        </div>
                    </Card>
                </div>
