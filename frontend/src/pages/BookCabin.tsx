...
export const BookCabin: React.FC<BookCabinProps> = ({ state, user, onBookCabin, onJoinWaitlist }) => {
  ...
  // Auto-select first floor if many floors exist
  useEffect(() => {
    if (availableFloors.length > 0 && activeFloor === 'All') {
      // If more than 1 floor, default to Floor 1 to reduce clutter
      if (availableFloors.length > 1) setActiveFloor(availableFloors[0]);
    }
    ...
    setTimeout(async () => {
      if (selectedCabin) {
        try {
          ...
          console.error(e);
          setPaymentStep('details'); // Go back to details or handle error state
        }
      }
    }, 1500);
  };
