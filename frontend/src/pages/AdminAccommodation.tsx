...
import { Home, Plus, MapPin, Edit2, Trash2, Phone, Image as ImageIcon, CheckCircle, ArrowRight, AlertCircle, Upload, Wallet, Loader2, MessageSquare, Calendar, Send, X, Sparkles } from 'lucide-react';
...
export const AdminAccommodation: React.FC<AdminAccommodationProps> = ({ state, onCreateAccommodation, onUpdateAccommodation, onDeleteAccommodation }) => {
    ...
    const handlePaymentSubmit = async () => {
        ...
        try {
            ...
            const errorMsg = e?.response?.data?.detail || "Error submitting payment. Please check all fields and try again.";
            alert(errorMsg);
        } finally {
            setSubmitting(false);
            // Keep wizard open on error so user can retry or go back
        }
    };
                    ...
                    {wizardStep === 3 && (
                        <div className="text-center space-y-4">
                            {!editingId ? (
                                ...
                                <>
                                    <div className="bg-indigo-100 p-4 rounded-lg inline-block"><Wallet className="w-8 h-8 text-indigo-600" /></div>
                                    <h3 className="font-bold text-lg">Listing Fee</h3>
                                    {/* Dynamic pricing from Super Admin settings (no hardcoding) */}
                                    <p className="text-gray-600">
                                        Pay ₹{state.settings?.payments?.featuredListingPrice || 'Contact Support'} to verify and list this property.
                                    </p>
                                    <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded">
                                        ⏳ Requires Super Admin approval after payment
                                    </p>
                                    <Button className="w-full" onClick={handlePaymentSubmit} disabled={submitting}>
                                        {submitting ? 'Processing...' : 'Pay & Submit for Approval'}
                                    </Button>
